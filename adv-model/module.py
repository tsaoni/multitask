...
class ModelTrainTemplate(pl.LightningModule):
    ...
    def __init__(
        ...
        **config_kwargs,
        ...
        if self.hparams.output_dir is None:
            # prefix = 'multitask_' if self.hparams.custom_model_name == 'multitask' else self.hparams.task + '_'
            prefix = self.hparams.task + '_'
            output_dir_name = prefix + 'tb={}_'.format(self.hparams.train_batch_size) + \
            ...
            self.output_dir = os.path.join('../models', self.hparams.custom_model_name, output_dir_name)
            Path(self.output_dir).mkdir(parents=True, exist_ok=True)
        else:
            self.output_dir = os.path.join('../models', self.hparams.custom_model_name, self.hparams.output_dir)
        ...
        self.step_count = 0
        self.config, self.tokenizer, self.model = self._get_model_from_argparse_args(
            config=config,
            tokenizer=tokenizer,
            model=model,
            **config_kwargs
        )
        config = [config for config in self.config.values() if config.model_id == 2][0] \
                        if multitask_id is not None else self.config
    ...
    def _task_specific_parameter_setting(self):
        if self.is_seq2seq:
            ...
            """ todo: extra arguments for bart model, later to set
            ...
            """
            config = self.model.config if self.multitask_id is None else self.model.models[1].config
            if config.decoder_start_token_id is None and isinstance(self.tokenizer, MBartTokenizer):
                self.decoder_start_token_id = self.tokenizer.lang_code_to_id[hparams.tgt_lang]
                self.model.models[1].config.decoder_start_token_id = self.decoder_start_token_id
            else:
                self.decoder_start_token_id = config.decoder_start_token_id

            self.eval_beams = config.num_beams if self.hparams.eval_beams is None else self.hparams.eval_beams
            assert self.eval_beams >= 1, f"got self.eval_beams={self.eval_beams}. Need an integer > 1"
            if self.hparams.eval_max_gen_length is not None:
                self.eval_max_length = self.hparams.eval_max_gen_length
            else:
                self.eval_max_length = config.max_length
    ...
    def configure_optimizers(self):
        ...
        optimizer_grouped_parameters, optimizer_grouped_parameters_name = [], []
        optimizer_params_state = dict()
        for n, p in model.named_parameters():
            optimizer_grouped_parameters_name.append(n)
            if not any(nd in n for nd in no_decay):
                optimizer_params_state[n] = dict(
                    params=p, 
                    weight_decay=self.hparams.weight_decay, 
                    lr=self.hparams.learning_rate,
                )
                optimizer_grouped_parameters.append({"params": p, "weight_decay": self.hparams.weight_decay, })
            else:
                optimizer_params_state[n] = dict(
                    params=p, 
                    weight_decay=0.0, 
        ...
        optimizer.param_groups_name = optimizer_grouped_parameters_name
        optimizer.params_state = optimizer_params_state
        self.opt = optimizer
    ...
    def _get_dataset(self, type_path) -> GeneralDataset:
        ...
        # reset if pad to max len
        self.eval_max_length = dataset.max_target_length if self.hparams.pad_to_max_len else self.eval_max_length
        return dataset
    ...
    def forward(self, input_ids, **kwargs):
        ''' want to freeze embedding but failed ? '''
        #if hasattr(self.model, 'freeze_parameters'):
        #    self.model.freeze_parameters(self.opt, self.multitask_id)
        # clip the gradient
        ...
        if self.multitask_id is None: 
            ...
            return self.model(input_ids, self.multitask_id, **kwargs)
    ...
    def _step(self, batch: dict) -> Tuple:
        ...
        if self.is_seq2seq:
            ...
            # extra data passed by dataloader, but not used in model input
            keyword_not_pass = ['src_texts', 'tgt_texts', ]
            batch_input = {key: value for key, value in batch.items() if key not in keyword_not_pass}
            # forward
    ...
    def validation_epoch_end(self, outputs, prefix="val") -> Dict:
        self.step_count += 1
        # store preds and target
        if self.is_seq2seq:
            for batch in outputs:
                for pred, target, src_txt, tgt_txt in zip(batch['preds'], batch['target'], batch['src_texts'], batch['tgt_texts']):
                    self.validation_result.append(dict(pred=pred, target=target, src_text=src_txt, tgt_text=tgt_txt))
        # calculate metrics
    ...
    def _inference_step(self, batch: dict) -> dict:
        ...
        if self.is_seq2seq:
            ...
            base_metrics.update(**{key: batch[key] for key in ['src_texts', 'tgt_texts']})
        
        elif self.hparams.model_mode == 'sequence-classification':
            model = self.model if self.multitask_id is None else self.model.models[self.multitask_id - 1]
            outputs = model(**batch)
    ...
    def _generate_output_ids(self, batch: dict):
        ...
        if self.multitask_id is None:
            ...
            model = self.model.models[self.multitask_id - 1]
        if isinstance(model, GPT2LMHeadModel):
            ...
            assert isinstance(model, seq2seq_model_type)
    ...
    def on_save_checkpoint(self, checkpoint: Dict[str, Any], filepath='checkpoint') -> None:
        ...
        if self.multitask_id is None:
            ...
            paths = [ Path(self.output_dir).joinpath("checkpoint-curr_best/victim_mode"),
             Path(self.output_dir).joinpath("checkpoint-curr_best/text2struct_mode") ]
            for i, save_path in enumerate(paths):
                self.model.models[i].config.save_step = self.step_count
                self.model.models[i].save_pretrained(save_path)
                self.tokenizer.save_pretrained(save_path)
                print('the suggested save_path is {}, saving to {}'.format(filepath, save_path))
                print('SAVING TO checkpoint {}'.format(save_path))
            ...
            torch.save(
                ...
                Path(self.output_dir).joinpath("checkpoint-curr_best/multitask_model.pt")
            )
...
class BaseModel(abc.ABC, nn.Module):
    ...
    def forward(self, x):
        # will not be called if your model have implement forward
        return x

    @staticmethod
    @abc.abstractmethod
    def add_model_specific_args(parser):
        raise NotImplementedError("you must implement the function 'add_model_specific_args'. ")

    @abc.abstractmethod
    def _get_config_args(self, src_args: argparse.Namespace) -> argparse.Namespace:
        raise NotImplementedError("you must implement the function 'get_config_args'. ")