...
class ModelTrainTemplate(pl.LightningModule):
    ...
    def __init__(
        ...
        dataset_cls_dict=None,
        # config=None,
        # tokenizer=None,
        # model=None,
        victim_config=None,
        struct2sent_config=None,
        victim_tokenizer=None,
        multitask_tokenizer=None, # multi-task tokenizer setting is already set
        struct2sent_tokenizer=None,
        victim_model=None,
        multitask_model=None, # multi-task model setting is already set
        struct2sent_model=None,
        **config_kwargs,
    ):
        super().__init__()
        ...
        if self.hparams.output_dir is None:
            ...
            output_dir_name = os.path.join('../models', self.hparams.output_dir)

        self.output_dir = dict()
        subdir_name = ['victim', 'multi-task', 'struct2sent']
        for name in subdir_name:
            self.output_dir[name] = os.path.join('../models', output_dir_name, name)
            Path(self.output_dir[name]).mkdir(parents=True, exist_ok=True)

            if len(os.listdir(self.output_dir[name])) > 3 and self.hparams.do_train:
                print('Output directory ({}) already exists and is not empty, overwrite to it...'\
                                                                    .format(self.output_dir[name]))

        # todo: check if it needs to save for each model
        ...
        self.data_dir = dict()
        data_dir_name = ['victim', 'struct2sent', 'sent2struct']
        hparams_dict = vars(self.hparams)
        data_dir_path_name = [hparams_dict[key + '_data_dir'] for key in data_dir_name]
        for name, path_name in zip(data_dir_name, data_dir_path_name):
            self.data_dir[name] = os.path.join('../data', path_name)
        """
        ...
        """
        self.dataset_kwargs: dict = dict()
        for name in data_dir_name:
            dataset_sub_kwargs = dict(
                data_dir=self.data_dir[name],
                max_source_length=hparams_dict[name + '_max_source_length']
            )
            self.dataset_kwargs[name] = dataset_sub_kwargs
        ...
        self.dataset_class = dataset_cls_dict
    ...
    def forward(self, input_ids, task_id, **kwargs):
        return self.model(input_ids, task_id, **kwargs)
...
class BaseModel(abc.ABC, nn.Module):
    ...
    @abc.abstractmethod
    def get_config_args(self, src_args: argparse.Namespace) -> argparse.Namespace:
